...
from app.auth.permissions import require_admin, require_student_owns
...
@router.get(
    "/stats/",
    response=CtiStatsOut,
    summary="Statistiques CTI VII.D3 (JSON)",
    auth=AdminJWTAuth(),
)
@require_admin
...
@router.get(
    "/export/", summary="Export rapport CTI (Excel VII.D3)", auth=AdminJWTAuth()
)
@require_admin
